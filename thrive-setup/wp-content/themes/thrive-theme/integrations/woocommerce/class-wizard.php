...
class Wizard {
	...
	public static function init() {
		...
		add_filter( 'thrive_theme_wizard_fetch_template', [ __CLASS__, 'wizard_template' ], 10, 3 );
	...
	public static function wizard_templates( $templates, $request ) {
		...
		switch ( $type ) {
			...
			case Main::FOOTER:
				...
				$request->set_param( 'type', str_replace( 'woo_', '', $type ) );
				/* remove "blank" templates for now and we will have to bring only the woo headers and footers */
				$templates = array_filter( $section_rest->get_cloud_sections( $request )->get_data()['data'], static function ( $template ) {
					return strpos( $template['post_title'], 'Blank' ) !== 0;
				} );

				/* Sort the templates in order to have the ones from woo first in the list */
				foreach ( $templates as $key => $template ) {
					$templates[ $key ]['order'] = ( isset( $template['is_woo'] ) && $template['is_woo'] === '1' ) ? - 1 : PHP_INT_MAX;
				}
	 ...
	 * @return array
	 */
	public static function wizard_template( $data, $type, $id ) {
		switch ( $type ) {
			case Main::HEADER:
			case Main::FOOTER:
